...
import { createEvent, updateEvent } from "../../api/userServices";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../app/store";
import { setAllEvents, setNewEvent } from "../../features/eventSlice";

function EventEditForm({ setShowModal,eventData }: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const newEvent = events.map((obj:any)=>{
                if(String(obj?._id)==String(eventData?._id)){
                   return formData;
                }

                return obj;
            })
            dispatch(setAllEvents(newEvent))
            setShowModal(false)
        } catch (err: any) {
